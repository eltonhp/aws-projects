...
def gera_dados_json(resultado_comparacao):
    ...
    for face_matches in resultado_comparacao:
        if (len(face_matches.get('FaceMatches'))) >= 1:
            perfil = dict(nome = face_matches['FaceMatches'][2]['Face']['ExternalImageId'],
                          faceMatch= round(face_matches['FaceMatches'][2]['Similarity'],2 ));
...
def publica_dados(dados_json):
     arquivo = s3.Object('fa-sites','dados.json');
